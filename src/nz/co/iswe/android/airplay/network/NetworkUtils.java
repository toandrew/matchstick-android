	...
	public byte[] getHardwareAddress() {
		try {
	    	...
	    	for(final NetworkInterface iface: Collections.list(NetworkInterface.getNetworkInterfaces())) {
                ...
                List<InetAddress> ialist = Collections.list(iface
                        .getInetAddresses());
                for (InetAddress addr : ialist) {
                    ...
                    try {
                        ...
                        if ((ifaceMacAddress != null) && (ifaceMacAddress.length == 6) && !isBlockedHardwareAddress(ifaceMacAddress)) {
                            LOG.info("Hardware address is " + toHexString(ifaceMacAddress) + " (" + iface.getDisplayName() + ")[" + addr.getHostAddress().toString() + "]");
