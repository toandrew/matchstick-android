...
public class AudioOutputQueue implements AudioClock {
	...
	private static final double MAX_VOLUME_IN_DB = 0;
	private static final double MIN_VOLUME_IN_DB = -30;
	private static final double MUTE_VOLUME_IN_DB = -144;
...
//	            }
	...
	private void setStereoVolume(float leftVolume, float rightVolume) {
	    LOG.info("before setStereoVolume() leftVolume: " + leftVolume + " rightVolume: " + rightVolume);
		
	    float lVolume = AudioTrack.getMaxVolume();
	    float rVolume = AudioTrack.getMaxVolume();
	    
	    if (leftVolume == MUTE_VOLUME_IN_DB) {
	        lVolume = AudioTrack.getMinVolume();
	    } else {
	        lVolume = (float) Math.abs((leftVolume - MIN_VOLUME_IN_DB)/MIN_VOLUME_IN_DB);
	    }
	    
        if (rightVolume == MUTE_VOLUME_IN_DB) {
            rVolume = AudioTrack.getMinVolume();
        } else {
            rVolume = (float) Math.abs((rightVolume - MIN_VOLUME_IN_DB)/MIN_VOLUME_IN_DB);
        }
        
		//validate left volume
		if(lVolume < AudioTrack.getMinVolume()){
		    lVolume = AudioTrack.getMinVolume();
		}
		if(lVolume > AudioTrack.getMaxVolume()){
		    lVolume = AudioTrack.getMaxVolume();
		}
		
		//validate right volume
		if(rVolume < AudioTrack.getMinVolume()){
		    rVolume = AudioTrack.getMinVolume();
		}
		if(rVolume > AudioTrack.getMaxVolume()){
		    rVolume = AudioTrack.getMaxVolume();
		}
		
		LOG.info("setStereoVolume() leftVolume: " + lVolume + " rightVolume: " + rVolume);
		
		audioTrack.setStereoVolume(lVolume, rVolume);
	}
...
//		LOG.info(" delay: " + delay );
		...
		if (delay < -packetSeconds) {
			...
			LOG.warning("Audio data arrived " + -(delay) + "[" + packetSeconds +"] seconds too late, dropping");
			return false;
		}
