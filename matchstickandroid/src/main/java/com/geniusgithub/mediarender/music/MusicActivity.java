...
public class MusicActivity extends BaseActivity implements
    ...
    private void refreshIntent(Intent intent) {
        ...
        LoaderHelper.syncDownLoadDrawable(mMediaInfo.getAlbumUri(), mHandler,
                LOAD_DRAWABLE_COMPLETE);
        ...
        if (need) {
            mLrcDownLoadHelper.syncDownLoadLRC(mMediaInfo.getTitle(),
                    mMediaInfo.getArtist(), this);
        }
        mUIManager.updateLyricView(mMediaInfo);
    ...
    class UIManager implements OnClickListener, OnSeekBarChangeListener,
        ...
        public void updateLyricView(DlnaMediaModel mMediaInfo) {
            log.e("updateLyricView song:" + mMediaInfo.getTitle() + ", artist:"
                    + mMediaInfo.getArtist());
        ...
        public void updateAlbumPIC(Drawable drawable) {
            Bitmap bitmap = ImageUtils.createRotateReflectedMap(mContext,
                    drawable);
            if (bitmap != null) {
        ...
        public void showPlayErrorTip() {
            Toast.makeText(MusicActivity.this, R.string.toast_musicplay_fail,
    ...
    private boolean checkNeedDownLyric(DlnaMediaModel mediaInfo) {
        String lyricPath = MusicUtils.getLyricFile(mediaInfo.getTitle(),
                mediaInfo.getArtist());
        if (lyricPath != null) {
    ...
    @Override
    public void lrcDownLoadComplete(boolean isSuccess, String song,
            String artist) {

        if (isSuccess && song.equals(mMediaInfo.getTitle())
                && artist.equals(mMediaInfo.getArtist())) {
